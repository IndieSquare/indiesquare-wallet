...
(function (root, factory) {
	...
	if (typeof Ti !== 'undefined') { // Titanium.
        getRandom = function(bits) {
            return globals.randomBytes;
        };
    } else if (typeof window !== 'undefined') { //a browser
        ...
        getRandom = function (bits) {
            var randomBytes = crypto.randomBytes(bits / 8), random = [];
    ...
    var Mnemonic = function (args, random) {
        var bits, i, l, w1, w2, w3, n;

        if (args == null || typeof args === 'undefined' || typeof args === 'number') {
			...
			for (var i = 0; i < args.length; i++) {
			    if(Mnemonic.words.indexOf(args[i]) > -1){}
			    else{
